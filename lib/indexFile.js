...
indexFile.sizeChunkInBytes = function (chunk, len) {
    const blob = new Blob([chunk.substr(0, len)]);
    return blob.size;
};
    ...
    fileReadStream.on("data", (chunk) => {
        ...
        do {
            ...
            if (existDelimiter) {
                ...
                buffer.writeInt32LE(position + indexFile.sizeChunkInBytes(chunk, delimiterPosition) + delimiter.length);
                indexWriteStream.write(buffer);
            }
        } while (existDelimiter);
        position += indexFile.sizeChunkInBytes(chunk);
